"""
Generic plotting algorithms for ROTSE-III QAs
"""
import numpy as np
import matplotlib.pyplot as plt

def rotse_qaplot(fig, plotconf, qadict, camera, expid, outfile):
    """
    ...
    if len(plotconfig) != 0:
        ...
        for p in range(nplots):
            ...
            if plottype == 'PATCH':
                ...
                grid=plot['GRID']
                patch=patchplot(ax,vals,plottitle,grid,heatmap)
                fig.colorbar(patch)
            if plottype == '2DPLOT':
                ...
                xtitle=plot['XTITLE']
                ytitle=plot['YTITLE']
                plot_2d(ax,xvals,yvals,plottitle,xtitle,ytitle,xlim,ylim)
            if plottype == '3DPLOT':
                xvals=np.array(qadict['METRICS'][plot['XVALS']])
                yvals=np.array(qadict['METRICS'][plot['YVALS']])
...
def patchplot(ax, vals, plottitle, grid, heatmap=None):
    """
...
def plot_3d(ax,xvals,yvals,zvals,plottitle,xtitle,ytitle,zlim=None,heatmap=None):
    """
